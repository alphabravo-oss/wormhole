...
type DiffOptions struct {
	ShowMetadata      bool
	PortableMetadata  bool
	IgnoreFileModTime bool
}

func (opts *DiffOptions) AddFlags(f *pflag.FlagSet) {
	f.BoolVar(&opts.ShowMetadata, "metadata", false, "print changes in metadata")
	f.BoolVar(&opts.PortableMetadata, "metadata-portable", false, "print portable metadata changes, ignoring inode, device, access time, and change time")
	f.BoolVar(&opts.IgnoreFileModTime, "ignore-file-mtime", false, "ignore file modification times when comparing metadata")
}

func metadataEqual(node1, node2 *data.Node, portable, ignoreFileModTime bool) bool {
	if !portable {
	...
	a.Inode, b.Inode = 0, 0
	a.DeviceID, b.DeviceID = 0, 0
	a.Content, b.Content = nil, nil
	a.Subtree, b.Subtree = nil, nil
	if ignoreFileModTime {
		a.ModTime, b.ModTime = time.Time{}, time.Time{}
	}
	if a.Type == data.NodeTypeDir && b.Type == data.NodeTypeDir {
		a.ModTime, b.ModTime = time.Time{}, time.Time{}
		a.Size, b.Size = 0, 0
		a.Links, b.Links = 0, 0
	}
...
func (c *Comparer) diffTree(ctx context.Context, stats *DiffStatsContainer, prefix string, id1, id2 restic.ID) error {
	...
	for dt := range data.DualTreeIterator(tree1, tree2) {
		...
		case node1 != nil && node2 != nil:
			...
			} else if (c.opts.ShowMetadata || c.opts.PortableMetadata) && !metadataEqual(node1, node2, c.opts.PortableMetadata, c.opts.IgnoreFileModTime) {
				mod += "U"
			}
