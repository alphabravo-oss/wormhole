...
func (opts *Options) AddFlags(f *pflag.FlagSet) {
	...
	f.BoolVar(&opts.WormholeParentLock, "wormhole-parent-lock", false, "use the repository lock held by the parent Wormhole process")
	if err := f.MarkHidden("wormhole-parent-lock"); err != nil {
		panic(err)
	}
